 ...
 * @param mid_message: True if the error occurred when reading incomplete message
 *
 * @return  - 0 on Timeout
...
static esp_err_t esp_mqtt_set_ssl_transport_properties(esp_transport_list_handle_t transport_list,
                                                       mqtt_config_storage_t *cfg)
{
    ...
    ESP_OK_CHECK(TAG, esp_mqtt_set_cert_key_data(ssl, MQTT_SSL_DATA_API_CLIENT_CERT, cfg->clientcert_buf,
                 ...
                 goto esp_mqtt_set_transport_failed);
    ESP_OK_CHECK(TAG, esp_mqtt_set_cert_key_data(ssl, MQTT_SSL_DATA_API_CLIENT_KEY, cfg->clientkey_buf,
                                                 cfg->clientkey_bytes),
                 goto esp_mqtt_set_transport_failed);
...
static esp_err_t esp_mqtt_check_cfg_conflict(const mqtt_config_storage_t *cfg, const esp_mqtt_client_config_t *user_cfg)
{
    ...
    bool ssl_cfg_enabled = cfg->use_global_ca_store || cfg->cacert_buf || cfg->clientcert_buf || cfg->psk_hint_key ||
                           cfg->alpn_protos;
    bool is_ssl_scheme = false;

    if (cfg->scheme) {
        is_ssl_scheme = (strncasecmp(cfg->scheme, MQTT_OVER_SSL_SCHEME, sizeof(MQTT_OVER_SSL_SCHEME)) == 0) ||
                        (strncasecmp(cfg->scheme, MQTT_OVER_WSS_SCHEME, sizeof(MQTT_OVER_WSS_SCHEME)) == 0);
    }
    ...
    if (!is_ssl_scheme && ssl_cfg_enabled) {
        if (cfg->uri) {
            ...
            ESP_LOGW(TAG, "SSL related configs set, but the transport protocol is a non-SSL scheme, transport = %d",
                     user_cfg->broker.address.transport);
        }
    ...
    if (cfg->uri && user_cfg->broker.address.transport) {
        ESP_LOGW(TAG, "Transport config set, but overridden by scheme from URI: transport = %d, uri scheme = %s",
                 user_cfg->broker.address.transport, cfg->scheme);
        ret = ESP_ERR_INVALID_ARG;
    }
...
static esp_err_t esp_mqtt_client_create_transport(esp_mqtt_client_handle_t client)
{
    ...
    if (client->config->scheme) {
        ...
        } else if ((strncasecmp(client->config->scheme, MQTT_OVER_SSL_SCHEME, sizeof(MQTT_OVER_SSL_SCHEME)) == 0) ||
                   (strncasecmp(client->config->scheme, MQTT_OVER_WSS_SCHEME, sizeof(MQTT_OVER_WSS_SCHEME)) == 0)) {
#if MQTT_ENABLE_SSL
...
esp_err_t esp_mqtt_set_config(esp_mqtt_client_handle_t client, const esp_mqtt_client_config_t *config)
{
    ...
    err = ESP_ERR_NO_MEM;
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->broker.address.hostname, &client->config->host),
                  goto _mqtt_set_config_failed);
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->broker.address.path, &client->config->path),
                  goto _mqtt_set_config_failed);
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->credentials.username,
                                              &client->mqtt_state.connection.information.username), goto _mqtt_set_config_failed);
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->credentials.authentication.password,
                                              &client->mqtt_state.connection.information.password), goto _mqtt_set_config_failed);

    if (!config->credentials.set_null_client_id) {
        if (config->credentials.client_id) {
            ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->credentials.client_id,
                                                      &client->mqtt_state.connection.information.client_id), goto _mqtt_set_config_failed);
    ...
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->broker.address.uri, &client->config->uri),
                  goto _mqtt_set_config_failed);
    ESP_MEM_CHECK(TAG, esp_mqtt_set_if_config(config->session.last_will.topic,
                                              &client->mqtt_state.connection.information.will_topic), goto _mqtt_set_config_failed);
    ...
    if (config->session.last_will.msg_len && config->session.last_will.msg) {
        ...
        memcpy(client->mqtt_state.connection.information.will_message, config->session.last_will.msg,
               config->session.last_will.msg_len);
    ...
    if (config->broker.verification.alpn_protos) {
        ...
        client->config->alpn_protos = calloc(client->config->num_alpn_protos + 1,
                                             sizeof(*config->broker.verification.alpn_protos));
    ...
    if (config->credentials.authentication.key_password && config->credentials.authentication.key_password_len) {
        ...
        memcpy(client->config->clientkey_password, config->credentials.authentication.key_password,
...
{
    esp_err_t ret = esp_event_post_to(client->config->event_loop_handle, MQTT_EVENTS, MQTT_USER_EVENT, event,
                                      sizeof(*event), 0);
#if MQTT_EVENT_QUEUE_SIZE > 1
...
static esp_err_t deliver_publish(esp_mqtt_client_handle_t client)
{
    ...
    if (client->mqtt_state.connection.information.protocol_ver == MQTT_PROTOCOL_V_5) {
        ...
        if (esp_mqtt5_get_publish_data(client, msg_buf, msg_read_len, &msg_topic, &msg_topic_len, &msg_data,
                                       &msg_data_len) != ESP_OK) {
...
static esp_err_t mqtt_process_receive(esp_mqtt_client_handle_t client)
{
...
#endif
        double transmitted;
        outbox_get_transmitted_time(client->outbox, msg_id, &transmitted);
...
#endif
            client->event.event_id = MQTT_EVENT_PUBACK;
            client->event.transmitted = transmitted;
            esp_mqtt_dispatch_event_with_msgid(client);
        }
    ...
    case MQTT_MSG_TYPE_PUBREC: {
        ...
        double transmitted;
        if(outbox_get_transmitted_time(client->outbox, msg_id, &transmitted) == ESP_OK){
            client->event.event_id = MQTT_EVENT_PUBREC;
            client->event.transmitted = transmitted;
            esp_mqtt_dispatch_event_with_msgid(client);
        } else {
            ESP_LOGE(TAG, "SQ: Failed to get transmitted time for msg_id=%d", msg_id);
        }
...
#endif
            client->event.event_id = MQTT_EVENT_PUBCOMP;
            esp_mqtt_dispatch_event_with_msgid(client);
        }
...
static void mqtt_delete_expired_messages(esp_mqtt_client_handle_t client)
{
    ...
    while ((msg_id = outbox_delete_single_expired(client->outbox, platform_tick_get_ms(),
                                                  OUTBOX_EXPIRED_TIMEOUT_MS)) >= 0) {
