...
/**
 * @brief Type definition for custom message id generation function.
 */
typedef int (*esp_mqtt_msg_id_fn_t)(void *ctx);

/**
 * @brief Sets a custom function to generate message ids for MQTT messages.
 * Caller must account for multithreading and ensure the uniqueness of message ids generated by the function.
 * 
 * @param client    *MQTT* client handle
 * @param fn        Custom function to generate message ids, must return a positive integer as message id, or
 *                  0 if message id generation failed. 
 * @param ctx       User context to be passed to the custom function when called.
 */
 ...
 * @param msg_id    message id, if set to 0, message id is generated by the client
 *
...
int esp_mqtt_client_publish(esp_mqtt_client_handle_t client, const char *topic,
                            const char *data, int len, int qos, int retain, int msg_id);
